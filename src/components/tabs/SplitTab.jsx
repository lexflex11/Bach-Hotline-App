  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
        <div style={{ ...C, textAlign:"center" }}><div style={{ fontSize:10, color:HOT, fontFamily:"'DM Sans',sans-serif", textTransform:"uppercase", letterSpacing:1, marginBottom:5 }}>Total Trip</div><div style={{ fontSize:26, fontWeight:900, color:PUNCH, fontFamily:"'Playfair Display',Georgia,serif" }}>${total.toFixed(0)}</div></div>
        <div style={{ ...C, textAlign:"center" }}><div style={{ fontSize:10, color:HOT, fontFamily:"'DM Sans',sans-serif", textTransform:"uppercase", letterSpacing:1, marginBottom:5 }}>Per Person</div><div style={{ fontSize:26, fontWeight:900, color:HOT, fontFamily:"'Playfair Display',Georgia,serif" }}>${perPerson.toFixed(0)}</div></div>
      </div>
      <div style={{ background:SOFT, border:`1.5px solid ${MID}`, borderRadius:12, padding:"11px 14px", marginBottom:14, fontFamily:"'DM Sans',sans-serif", fontSize:12, color:DARK }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:14 }}>
        {["summary","expenses","members"].map(v => (
          <button key={v} onClick={()=>setView(v)} style={{ flex:1, padding:"9px", borderRadius:50, border:view===v?`2px solid ${HOT}`:`1.5px solid ${BORDER}`, background:view===v?HOT:WHITE, color:view===v?WHITE:HOT, fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:700, cursor:"pointer" }}>
        ...
        return (
          <div key={m.id} style={{ ...C, marginBottom:8, display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              {isBride && <div style={{ fontSize:11, color:GOLD, fontFamily:"'DM Sans',sans-serif" }}>🎉 Pays nothing!</div>}
            </div>
            <div style={{ textAlign:"right" }}>
              ...
              <button onClick={()=>toggle(m.id)} style={{ fontSize:10, padding:"3px 9px", borderRadius:20, border:"none", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:700, background:m.paid?`rgba(46,125,50,0.1)`:SOFT, color:m.paid?GREEN:HOT, marginTop:3 }}>
      ...
      {view==="expenses" && expenses.map(e => (
        <div key={e.id} style={{ ...C, marginBottom:9, display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:11, color:HOT, fontFamily:"'DM Sans',sans-serif", opacity:0.75 }}>${(e.amount/(members.length-1)).toFixed(0)}/person</div>
          </div>
        ...
        <div>
          {members.map(m => (
            <div key={m.id} style={{ ...C, marginBottom:8, display:"flex", alignItems:"center", gap:12 }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, fontWeight:700, fontFamily:"'Playfair Display',Georgia,serif", color:DARK }}>{m.name}</div>
                <div style={{ fontSize:11, color:HOT, fontFamily:"'DM Sans',sans-serif", opacity:0.75 }}>{m.name.includes("Bride")?"Queen — pays nothing 👑":`Owes: $${perPerson.toFixed(0)}`}</div>
              </div>
              <button onClick={()=>toggle(m.id)} style={{ fontSize:11, padding:"5px 12px", borderRadius:20, border:`1.5px solid ${m.paid?"rgba(46,125,50,0.3)":MID}`, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:700, background:m.paid?`rgba(46,125,50,0.08)`:SOFT, color:m.paid?GREEN:HOT }}>
                {m.paid?"Paid":"Unpaid"}
              </button>
            </div>
          ))}
