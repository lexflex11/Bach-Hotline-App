  ...
  return (
    <div>
      ...
      <div style={{borderRadius:20,padding:"22px 18px",background:`linear-gradient(135deg,${SOFT},${MID})`,border:`1.5px solid ${MID}`,marginBottom:14,textAlign:"center"}}>
        <div style={{fontSize:11,color:HOT,fontFamily:"'DM Sans',sans-serif",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>
        ...
        <div style={{fontSize:14,color:HOT,fontFamily:"'DM Sans',sans-serif",fontWeight:600,marginBottom:6}}>🕐 {currentItem.time}</div>
        <div style={{fontSize:12,color:DARK,fontFamily:"'DM Sans',sans-serif",opacity:0.75,marginBottom:14}}>{currentItem.address}</div>
        {currentItem.note && (
          <div style={{background:WHITE,borderRadius:10,padding:"10px 12px",marginBottom:14,fontSize:12,color:DARK,fontFamily:"'DM Sans',sans-serif",textAlign:"left",lineHeight:1.5}}>
      ...
      {stops.map((stop,i)=>(
        <button key={i} onClick={()=>setCurrentStop(i)} style={{...C,display:"flex",alignItems:"center",gap:12,cursor:"pointer",width:"100%",marginBottom:8,textAlign:"left",background:currentStop===i?SOFT:WHITE,border:currentStop===i?`2px solid ${HOT}`:`1.5px solid ${BORDER}`}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:HOT,fontFamily:"'DM Sans',sans-serif",marginTop:2,opacity:0.75}}>{stop.time} · {stop.address}</div>
          </div>
